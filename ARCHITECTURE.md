...
MCP client (Claude Desktop / Code / …)
  ...
  └───────────────┬───────────────────────────────┘
                  ▼  HTTPS /api/v2/...
            pfSense REST API v2 (pkg-RESTAPI v2.10.2)
```
...
- `scripts/generate_contract.py` distills the upstream OpenAPI spec (a
  ...
  (`tests/contract/contract-v2.10.2.json`): valid field names, JSON types, enum
...
pkg-RESTAPI **v2.10.2**. See the README compatibility matrix; legacy names such as
